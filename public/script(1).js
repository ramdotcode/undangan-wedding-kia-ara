...
(function ($) {
  ...
  var WdkSticky = function ($scope, $) {
    var WdkStickySection = $scope.find('.wdk-sticky-section-yes').eq(0);

    WdkStickySection.each(function (i) {
      var dataSettings = $(this).data('settings');
      $.each(dataSettings, function (index, value) {
        if (index === 'wdk_sticky_top_spacing') {
          $scope.find('.wdk-sticky-section-yes').css('top', value + 'px');
        }
      });
    });
    ...
    var columnClass = $scope.find('.wdk-column-sticky');
